...
export function InternalUnifiedQuoteEmail({ payload, submittedAt, attachmentName }: Props) {
  const techGroups = payload.technicalItems.reduce<
    Record<string, { titleEs: string; items: QuoteUnifiedValues["technicalItems"] }>
  >((acc, item) => {
  ...
  const provGroups = payload.provisionsItems.reduce<
    Record<string, { name: string; items: QuoteUnifiedValues["provisionsItems"] }>
  >((acc, item) => {
        ...
        <div style={cardStyle}>
          <div style={headerStyle}>
            ...
            <h1 style={{ fontSize: "20px", margin: "6px 0 0", fontWeight: 600 }}>
              Nueva cotización — Combinada / New Quote — Combined
            </h1>
          ...
          {/* ── Technical Supplies Section ── */}
          {payload.technicalItems.length > 0 && (
            <>
              <h3 style={sectionTitleStyle}>
                Suministros Técnicos / Technical Supplies ({payload.technicalItems.length} ítems)
              </h3>
              <table style={itemsTableStyle} cellPadding={0} cellSpacing={0}>
                ...
                <tbody>
                  {Object.entries(techGroups).map(([catId, group]) => (
                    <React.Fragment key={catId}>
                      <tr>
                        <td colSpan={4} style={catRowStyle}>{group.titleEs}</td>
                      </tr>
          ...
          {/* ── Provisions Section ── */}
          {(payload.provisionsItems.length > 0 || attachmentName) && (
            <>
              <h3 style={sectionTitleStyle}>
                Provisiones / Provisions
                {payload.provisionsItems.length > 0
                  ? ` (${payload.provisionsItems.length} ítems)`
                  : ""}
              </h3>

              {attachmentName && (
                <div style={{ margin: "0 24px 12px", padding: "10px 14px", background: "#fafaf6", border: `1px solid ${border}`, borderRadius: "4px", fontSize: "13px", color: charcoal }}>
                  📎 Archivo adjunto: <strong>{attachmentName}</strong>
                </div>
              )}
                ...
                <table style={itemsTableStyle} cellPadding={0} cellSpacing={0}>
                  <thead>
                    <tr>
                      <th style={itemsThStyle}>Producto / Product</th>
                      <th style={{ ...itemsThStyle, width: "60px", textAlign: "right" }}>Cant.</th>
                      <th style={{ ...itemsThStyle, width: "80px" }}>Unidad</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(provGroups).map(([catKey, group]) => (
                      <React.Fragment key={catKey}>
                        <tr>
                          <td colSpan={3} style={catRowStyle}>{group.name}</td>
                        </tr>
                        {group.items.map((item) => (
                          <tr key={item.id}>
                            <td style={itemsTdStyle}>{item.name}</td>
...
export default InternalUnifiedQuoteEmail;
